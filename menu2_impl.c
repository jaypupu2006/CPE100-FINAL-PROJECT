...
int open_or_create_daily(const char *daily_path) // เปิดหรือสร้างไฟล์ใหม่  const char ให้เป็นค่าคงที่ที่แก้ไขไม่ได้
{
    ...
    fprintf(fp, "ID|ชื่อ-นาสกุล|ชื่อเล่น|เพศ|จำนวนลูก|ค่าสนาม|ยอดรวม|ยอดที่ชำระแล้ว|ยอดชำระรายการค้าง|สถานะการชำระ\n"); 
    fclose(fp);
    return 1;
}
...
int summarize_daily_and_write(const char *daily_path, const char *out_path, int full)
{
    ...
    {
        fprintf(fo, "วันที่|%s\n", date_only);
        fprintf(fo, "ลำดับ|ชื่อเล่น|ID|จำนวนลูก|ยอดทั้งหมด|ยอดที่ชำระแล้ว|ยอดชำระรายการค้าง|คงค้างวันนี้|วิธีชำระ\n");
    }
    else
    {
        fprintf(fo, "วันที่|%s\n", date_only);
    ...
    while (fgets(line, sizeof(line), fp))
    {
                   ...
                   &paid_today, &paid_os, method_today) == 10)
        {
            ...
            if (full)
            {
                ...
                int unpaid_row = amount_today - paid_today;
                if (unpaid_row < 0)
                    unpaid_row = 0;
